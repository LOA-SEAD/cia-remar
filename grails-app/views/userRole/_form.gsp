...
<div class="fieldcontain ${hasErrors(bean: userRoleInstance, field: 'role', 'error')} required">
	...
	<g:select id="role" name="role.id" from="${br.ufscar.sead.loa.remar.Role.list()}" optionKey="id" required="" value="${userRoleInstance?.role?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: userRoleInstance, field: 'user', 'error')} required">
	...
	<g:select id="user" name="user.id" from="${br.ufscar.sead.loa.remar.User.list()}" optionKey="id" required="" value="${userRoleInstance?.user?.id}" class="many-to-one"/>
